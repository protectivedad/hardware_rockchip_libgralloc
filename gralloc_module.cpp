...
int g_MMU_stat = 0;
#define RK_GRALLOC_VERSION "1.1.9"
...
static int gralloc_device_open(const hw_module_t *module, const char *name, hw_device_t **device)
{
    ...
    AINF("gralloc open device '%s' - version '%s' - build at '%s'- '%s'.",
        name,
        RK_GRALLOC_VERSION,
        __TIME__,
        __DATE__);
...
static int gralloc_unlock(gralloc_module_t const *module, buffer_handle_t handle)
{
	...
	else if (hnd->flags & private_handle_t::PRIV_FLAGS_USES_ION && hnd->writeOwner)
	{
		...
		private_module_t *m = NULL;
                  ...
                  if (module == NULL) {
                        if (hw_get_module(GRALLOC_HARDWARE_MODULE_ID, (const hw_module_t **)&pmodule) < 0)
                              ALOGE("%s: hw_get_module failed",__func__);
                  } else {
                        pmodule = (hw_module_t*)module;
                  }

                  if (pmodule) {
                        m = reinterpret_cast<private_module_t *>(pmodule);
                        if (ion_sync_fd(m->ion_client, hnd->share_fd) < 0)
                              ALOGE("%s: ion_sync_fd failed!",__func__);
                  } else {
                        ALOGE("%s: pmodule is NULL!",__func__);
                  }
